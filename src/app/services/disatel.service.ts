...
export class DisatelService {
  ...
  async getOrdenTrabajo<T>( ot ){
    this.datosUsuario = await this.storage.get('datos');
    console.log(`https://${this.datosUsuario.dominio}/${disatelUrl}orden&ot=${ot}`);
  ...
  async setChecklist<T>( ot, vehiculo, entrega, fechahora){
    this.datosUsuario = await this.storage.get('datos');
    return this.http.get<T>(`https://${this.datosUsuario.dominio}/${disatelEjecutar}finaliza_checklist&ot=${ot}&vehiculo=${vehiculo}&entrega=${entrega}&usuario=${this.datosUsuario.codigo}&fecha_hora=${fechahora}`);
  }
